...
function CalicutResorts() {
    const navigate = useNavigate()
    const navigateResort = (id) => {
        navigate(`/calicutresort/${id}`)
    }

    const [resort, setResort] = useState([])
    const handleNextImage = () => {
        document.getElementById('calicut').scrollLeft -= 500
    }

    const handlePrevImage = () => {
        document.getElementById('calicut').scrollLeft += 5500
    }

    const fetchHoneyMoonPackage = async () => {
        const response = await axios.get('https://trekfinale.onrender.com/packages/calicut');
            ...
            <div className=' pb-14 w-full h-auto bg-[#EBEBEB]  '>
                <div className=' flex w-[1150px] mx-auto  flex-col gap-3'>
                    ...
                    <div className='flex justify-between'>
                        <h1 className=' font-semibold text-4xl flex gap-2'>Best Calicut Resort</h1>
                    ...
                    <div id='calicut' className='relative scroll-smooth flex gap-3 overflow-scroll hide-scrollbar rounded-2xl mt-5'>
                        {resort.map((items) => {
        ...
        <div className=' w-full h-auto bg-[#EBEBEB] mt-[-8px]'>
            <div className=' w-full h-auto bg-[#EBEBEB]  py-5'>
                <div className=' px-5'>
                    ...
                    <h1 className=' text-xl font-bold flex inline-flex gap-1'>Best Calicut Resorts <span><img src='' alt="" width={30} /></span></h1>
