...
function KannurResorts() {
    const navigate = useNavigate()
    const navigateResort = (id) => {
        navigate(`/kannurresort/${id}`)
    }

    const [resort, setResort] = useState([])
    const handleNextImage = () => {
        document.getElementById('kannur').scrollLeft -= 500
    }

    const handlePrevImage = () => {
        document.getElementById('kannur').scrollLeft += 1500
    }

    const fetchHoneyMoonPackage = async () => {
        const response = await axios.get('https://easytuitionapi.cloud/packages/kannur');
            ...
            <div className=' pb-14 w-full h-auto bg-[#EBEBEB]  '>
                <div className=' flex w-[1150px] mx-auto  flex-col gap-3'>
                    ...
                    <div className='flex justify-between'>
                        <h1 className=' font-semibold text-4xl flex gap-2'>Best Kannur Resort</h1>
                    ...
                    <div id='kannur' className='relative scroll-smooth flex gap-3 overflow-scroll hide-scrollbar rounded-2xl mt-5'>
                        {resort.map((items) => {
        ...
        <div className=' w-full h-auto bg-[#EBEBEB] mt-[-8px]'>
            <div className=' w-full h-auto bg-[#EBEBEB]  py-5'>
                <div className=' px-5'>
                    ...
                    <h1 className=' text-xl font-bold flex inline-flex gap-1'>Best Kannur Resorts <span><img src='' alt="" width={30} /></span></h1>
